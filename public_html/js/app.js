    ...
    $scope.submit = function () {
        ...
        if($scope.form.email && $scope.form.name && $scope.form.password && $scope.form.password2 && $scope.form.address && $scope.form.mobile){
            ...
            if(!email.test($scope.form.email)){
                $scope.error.push("Bele Email olmur!");
                $scope.ok=false;
            }
            
            if(!namecheck.test($scope.form.name)){
               $scope.error.push("Adivi duzelt!");
               $scope.ok=false;
            }
            
            if(!password.test($scope.form.password)){
                $scope.error.push("Shifre yalnishdir");
                $scope.ok=false;
            }
            
            if($scope.form.password!==$scope.form.password2){
                $scope.error.push("Tekrar Shifre uygun deyil");
                $scope.ok=false;
            }
            if(!numberc.test($scope.form.mobile)){
                 $scope.error.push("Nomre yalnishdir");
                 $scope.ok=false;
            }
            if(!unvanc.test($scope.form.address)){
                 $scope.error.push("Unvan yalnishdir");
                 $scope.ok=false;
            }
        }
        else{
            $scope.error.push("Sahelerin hamisini doldur");
            $scope.ok=false;
        }
        if($scope.ok){
...
app.controller("NewProductController",function($scope,$http,FileUploader){
   ...
   $scope.getSubs = function(){
    ...
    $scope.submit = function(product){
        $scope.errors = [];
        if(!$scope.cansend){
            $scope.errors.push("Gozle bir deqiqe");
        }
        else
        if((product.name_az!='' ||product.name_ru!=''||product.name_en!='') && (product.text_az!='' ||product.text_ru!=''||product.text_en!='')){
            ...
            if (selected.length>0){
                if($scope.images.queue.length==0){
                    $scope.errors.push('En azi bir shekil elave ele');
                }
                else{
                    $scope.cansend = false;
                ...
                $scope.errors.push("En azi bir kategoriya selcilmelidi");
                $scope
            }
